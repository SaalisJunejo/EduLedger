/**
 * Deploys WorkloadLedger (Module 3 - Faculty Workload Ledger anchor)
 * to the local Hardhat network, grants BACKEND_ROLE to the deployer account
 * (which acts as the trusted backend signer during local development, the
 * same access pattern as AttendanceLedger), and exports the deployed
 * address + ABI to deployed/WorkloadLedger.json for the backend.
 *
 ...
 *   # or: npx hardhat run scripts/deploy-workload.js --network localhost
 */
const fs = require("fs");
const path = require("path");
const hre = require("hardhat");

const CONTRACT_NAME = "WorkloadLedger";
...
async function main() {
  ...
  // Same access pattern as AttendanceLedger: for now the deployer doubles
  // as the trusted backend signer; later the admin can grant BACKEND_ROLE
  // to the real backend's signing address.
  console.log("\nGranting roles:");
